...
@export  var shake_strength = Vector2(10, 10)
...
func _ready():
	...
	_duration_timer.connect("timeout", Callable(self, "stop"))
	set_process(false)


func _process(delta):
	offset.x = randf_range(-shake_strength.x, shake_strength.x)
	offset.y = randf_range(-shake_strength.y, shake_strength.y)
